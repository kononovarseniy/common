...
// IntervalValueUtils::value_inside_allowed_range

TEST(IntervalValueUtilsTest, value_inside_allowed_range)
{
    using Utils = IntervalValueUtils<NoBuiltinOperatorsInt, LessOnly>;
    ...
    EXPECT_TRUE(Utils::value_inside_allowed_range(mid));
    EXPECT_TRUE(Utils::value_inside_allowed_range(below_min));
    EXPECT_TRUE(Utils::value_inside_allowed_range(above_max));
}

TEST(IntervalValueUtilsTest, value_inside_allowed_range_default_integral)
{
    using Utils = IntervalValueUtils<s32, IntervalValueTraits<s32>>;
    EXPECT_TRUE(Utils::value_inside_allowed_range(std::numeric_limits<s32>::min()));
    EXPECT_TRUE(Utils::value_inside_allowed_range(std::numeric_limits<s32>::max()));
    EXPECT_TRUE(Utils::value_inside_allowed_range(0));
}
